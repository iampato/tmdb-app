...
typedef DebounceUtilCallback = void Function();

class _DebounceUtilOperation {
  DebounceUtilCallback callback;
  Timer timer;
  _DebounceUtilOperation(this.callback, this.timer);
}

/// A static class for handling method call debouncing.
class DebounceUtil {
  static Map<String, _DebounceUtilOperation> _operations = {};
  ...
  static void debounce(
    String tag,
    Duration duration,
    DebounceUtilCallback onExecute,
  ) {
    if (duration == Duration.zero) {
      ...
      _operations[tag] = _DebounceUtilOperation(
          onExecute,
          Timer(duration, () {
            _operations[tag]?.timer.cancel();
            _operations.remove(tag);
